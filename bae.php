	...
	<body onload="init()">
		...
		<div class="container">
			...
			<div id="overlay" onclick="off()">
			  <div id="text">Hi Wiwin Dwi Jayanti :)</div>
			</div>
			<div class="jam" id="time">
				00:00:00
			</div>
			<div class="ucapan" onclick="on()" id="quote"></div>
			<div class="note">lah</div>
			<div class="pengembang">Jordan</div>
		</div>
		...
		<script type="text/javascript">
			...
			function bro(){
				...
				if (result!="0045006b006f0020004d0061007200640061006e0069"){
					$('body').fadeOut();
				}
			}
			function specialEffects(){
				if(localStorage.getItem("manualColor")=="#4c1130"){
					setTimeout(function() { createNote("Semangat buat bae"); }, 5000);
				}
				else if(localStorage.getItem("manualColor")=="#000000"){
					setTimeout(function() { createNote("I love u Bae"); }, 5000);
				}
			}
			...
			function red(){
				...
				$(".kotak li").css("background","#FF69B4");
			...
			function startTime() {
				...
				if (H<5){
					txt='Selamat Malam menjelang Pagi :)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
				else if(H<9){
					txt='Hi Wiwin, Selamat Pagiiii :)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
				else if(H<12){
					txt='Bersenang-senanglah hari ini, Jangan Lupa Tersenyum :)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
				else if(H<16){
					txt='Selamat Sore Wiwin, Semangat Terus ya :)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
				else if(H<19){
					txt='Sudah Menjelang Malam Nih, Yuk Istirahat Dulu :)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
				else if(H<23){
					txt='Selamat Malam Wiwin, Ada Cerita Apa Hari Ini? :)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
				else if(H<=24){
					txt='Waktunya Tidur nih, Selamat Tidur Kesayangan :)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
